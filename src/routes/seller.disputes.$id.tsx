...
export const Route = createFileRoute("/seller/disputes/$id")({
...
function SellerDispute() {
  ...
  const apiBase = useMemo(() => {
    ...
    return (envUrl ? envUrl.replace(/\/$/, "") : "/api") as string;
  }, []);
  ...
  const open = async () => {
    try {
      await store.advanceTx(id, "DISPUTED");
  ...
  if (!tx) {
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center p-6">
        ...
        <Link to="/seller/orders" className="mt-4 text-sm text-primary font-semibold">
          Back to orders
        </Link>
      </div>
    );
    ...
    <div className="min-h-dvh px-5 pb-6 pt-12">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 text-xs text-muted-foreground">
              The transaction is locked while the dispute is active. Provide only verifiable details.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-3xl border border-border bg-card p-5">
        ...
        <div className="mt-2 grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <div className="font-semibold">{tx.buyerName}</div>
          </div>
      ...
      <div className="mt-4 rounded-3xl border border-border bg-card p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="text-sm font-semibold">Recommended response</div>
            <div className="mt-1 text-xs text-muted-foreground">
              Include shipping proof, delivery attempts, and any relevant notes tied to the transaction reference.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-3xl border border-border bg-card p-5">
        ...
        {!dispute ? (
          <button
            onClick={open}
            className="mt-3 flex h-14 w-full items-center justify-center rounded-2xl bg-primary font-semibold text-primary-foreground tap-scale shadow-[var(--shadow-glow)]"
          >
            Open dispute
          </button>
        ) : (
      ...
      {loading ? null : dispute ? (
        ...
        <div className="mt-6 rounded-3xl border border-border bg-card p-5 text-sm text-muted-foreground">
