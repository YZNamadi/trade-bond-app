...
export const Route = createFileRoute("/seller/settings/support")({
    ...
    <div className="min-h-dvh px-5 pb-6 pt-12">
      ...
      <div className="mt-6 rounded-3xl border border-border bg-card p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="text-sm font-semibold">Seller support</div>
            <div className="mt-1 text-xs text-muted-foreground">
              For disputes, include the transaction reference and screenshots of shipping proof if available.
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="mt-6">
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card divide-y divide-border">
          ...
          <Row icon={<ShieldAlert className="h-4 w-4" />} label="Report a dispute issue" onClick={() => document.getElementById("tt-support-form-seller")?.scrollIntoView({ behavior: "smooth" })} />
          <Row icon={<Bug className="h-4 w-4" />} label="Report a bug" onClick={() => document.getElementById("tt-support-form-seller")?.scrollIntoView({ behavior: "smooth" })} />
        </div>
      </section>

      <section className="mt-6" id="tt-support-form-seller">
